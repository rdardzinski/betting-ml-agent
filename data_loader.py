...
def get_next_matches():
    ...
    for league, url in FOOTBALL_LEAGUES.items():
        try:
            ...
            if not all(col in df.columns for col in required_cols):
                print(f"[WARN] {league} missing required columns, skipping")
                continue
    ...
    if not frames:
        print("[INFO] Brak danych piłki nożnej")
        return pd.DataFrame()
...
def get_nba_games():
    """
    Dane historyczne NBA – CSV z GitHub
    Kolumny: Date, HomeTeam, AwayTeam, HomeScore, AwayScore
    """
    try:
        ...
        print(f"[WARN] NBA data not loaded: {e}")
        return pd.DataFrame()
